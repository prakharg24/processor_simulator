...
//change this to add any instruction
...
int whattodo(int *ins)
{
    ...
    {
        int opcc[10];
        subarr(ins, 6, 15,opcc);
        int fu[5];
        subarr(ins, 21, 25,fu);
        int vall =  bintoint(opcc,10);
        int vallu = bintoint(fu,5) ;
        if(vall==0 && vallu==0)
        {
            return 25 ;
        }
    ...
    {
        int opcc[5];
        subarr(ins, 11, 15,opcc);
        int fu[5];
        subarr(ins, 21, 25,fu);
        int vall =  bintoint(opcc,5);
        int vallu = bintoint(fu,5) ;
        if(vall==0 && vallu==0)
        {
            return 28 ;
...
{
	if(fun==7)
		return 4;       //1 input 11-15
	if(fun<=6 || (fun>=8 && fun<=10) || fun==22 || fun==17 || fun==18 || fun==30)
	{
		return 1;       //2 input 6-10 and 11-15
	}
	else if(fun==21 || fun==25 || fun==26 || fun==27)
		return 3;    // no input
	else
		return 2;     // 1 imput 6-10
}

int outtype(int fun)
{
	if(fun==1 || fun==2|| (fun>=5 && fun<=9) || fun==22 || fun==25 || fun==30)
	{
		return 1;   //output 16-20
	}
	else if(fun==15 || fun==16 || fun==23 || (fun>=19 && fun<=21))
		return 2;   //output 11-15
	else
		return 3; //no output + r31 in jal + reg in jalr
}
...
void *pcwala(void *data)
{
	...
	if(in->en)
	{
		...
		if((func==15 || func==16) && iff_temp.en==1)        // spcl case of ld
		{
			int rt[5];
	...
	if(in->en)
	{
	    ...
	    else
	    {
            ...
            if(func>=26 && func<=29)
            {
                if(func==26 || func==27)
                {
                    nextpcvalue = (op2*4-4194304)/4;
                }
                else
                {
                    nextpcvalue = (op1*4 - 4194304)/4;
                }
            }
		    if((func>=10 && func<=14) || (func>=26 && func<=29))
		    {
		    	if((func==10 && op1==df) || ( func==11 && op1>=0) || (func==12 && op1>0) || (func==13 && op1<=0) || (func==14 && op1<0) || (func==26) || func==27 || func==28 || func==29)
			    {
		    		ex.en = -1;
		    		res = op2;
		    		brr=1;
    ...
    for(int i=0;i<5;i++)
    {
   	...
   	if(ex.en==-1)
   	{
   		if(id.en^iff.en==1)
   		{
            if(jal!=-1)
            {
                rf[jal] = pc.pc - 1 + 4193404/4;//TODO chk write pc goes inside
            }
   		...
   		else if(id.en==0 && iff.en==0)
   		{
            if(jal!=-1)
            {
                rf[jal] = pc.pc + 4193404/4;//TODO chk write pc goes inside
            }
   			pc.pc = pc.pc + ex.res ;
   		}
   		else
   		{
            if(jal!=-1)
            {
                rf[jal] = pc.pc - 2+ 4193404/4;//TODO chk write pc goes inside
            }
