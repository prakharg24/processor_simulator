...
void i_init(int exis,int size, int assoc,int bsize,int wb)
{
	...
	c1.n_sets = size / (words_per_block * c1.associativity);
	int wpb = bsize / WORD_SIZE;
	c1.index_mask = bsize * c1.n_sets - 1;
	c1.index_mask_offset = LOG2(WORD_SIZE) + LOG2(wpb) ;
...
void d_init(int exis,int size, int assoc,int bsize,int wb)
{
	...
	c2.n_sets = size / (words_per_block * c2.associativity);
	int wpb = bsize / WORD_SIZE;
	c2.index_mask = bsize * c2.n_sets - 1;
	c2.index_mask_offset = LOG2(WORD_SIZE) + LOG2(wpb) ;
